...
from analysis_core.vendor.models import Candle, Symbol
...
class BybitPublicClient:
    ...
    async def historical_completed_candles(
        self,
        symbol: str,
        *,
        timeframe: Literal["1m", "3m", "5m", "15m", "30m", "1h", "2h", "4h"],
        ...
        """Return a bounded historical window for retrospective evaluation only."""

        if start_at.tzinfo is None or end_at.tzinfo is None or end_at <= start_at:
            raise ValueError("historical candle bounds must be aware and increasing")
        interval, duration = {
            "1m": ("1", timedelta(minutes=1)),
            "3m": ("3", timedelta(minutes=3)),
            "5m": ("5", timedelta(minutes=5)),
        ...
        result, observed_at = await self._get_result(
            "/v5/market/kline",
            {
                ...
                # Bybit interprets end as an inclusive candle open timestamp. Use
                # the final millisecond inside the requested close-time window so
                # the next candle cannot consume one slot from the bounded limit.
                "end": int(end_at.timestamp() * 1000) - 1,
                "limit": max(1, min(limit, 1000)),
            },
        )
        if result.get("symbol") != symbol:
            raise BybitPublicError("Bybit historical kline symbol does not match request")
        rows = result.get("list")
        if not isinstance(rows, list):
            raise BybitPublicError("Bybit historical kline list is invalid")
        candles = [_candle(symbol, timeframe, duration, row, observed_at) for row in rows]
        completed = [
            candle
            for candle in candles
            if candle.completed and candle.open_time >= start_at and candle.close_time <= end_at
        ]
        completed.sort(key=lambda candle: candle.open_time)
        return tuple(completed)
    ...
    async def recent_candles(
        ...
        *,
        ...
        if not 1 <= limit <= 10000:
            raise ValueError("Candle window limit must be between 1 and 10000")
        candles: list[Candle] = []
        end_ms: int | None = None
        while len(candles) < limit:
            count = min(1000, limit - len(candles))
            params: dict[str, str | int] = {
                "category": "linear",
                "symbol": symbol,
                "interval": interval,
            ...
            if end_ms is not None:
                params["end"] = end_ms
            result, observed_at = await self._get_result("/v5/market/kline", params)
            if result.get("symbol") != symbol:
                raise BybitPublicError("Bybit kline symbol does not match request")
            rows = result.get("list")
            if not isinstance(rows, list):
            ...
            if (
                ...
                or (candles and page[-1].open_time + duration != candles[0].open_time)
            ):
                raise BybitPublicError("Invalid candle pagination continuity")
            candles = page + candles
            end_ms = int(page[0].open_time.timestamp() * 1000) - 1
            if len(page) < count:
                break
    ...
    async def orderbook(self, symbol: str, *, limit: int = 50) -> BybitOrderBook:
        ...
        return BybitOrderBook(
            symbol=symbol,
            observed_at=_timestamp(result.get("ts"), "orderbook snapshot time"),
...
def _public_trade(symbol: str, value: object) -> BybitPublicTrade:
    ...
    if value.get("symbol") != symbol:
        raise BybitPublicError("Bybit public trade symbol does not match request")
    side = value.get("side")

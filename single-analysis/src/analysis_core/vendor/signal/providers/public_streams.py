...
from analysis_core.vendor.signal.domain.enums import ToolStatus
from analysis_core.vendor.signal.domain.models import Symbol


def aware(value: datetime) -> datetime:
    if value.tzinfo is None or value.utcoffset() is None:
        raise ValueError("Public stream timestamp must be timezone-aware")
    return value
...
class LiquidationWindow(BaseModel):
    ...
    @field_validator("generated_at", "coverage_start", "coverage_end")
    @classmethod
...
class PublicStreamCache:
    ...
    def mark_subscribed(self, symbol: str, *, at: datetime | None = None) -> None:
        now = aware(at or datetime.now(UTC))
    ...
    def heartbeat(self, symbol: str, *, at: datetime | None = None) -> None:
        self._heartbeat[symbol] = aware(at or datetime.now(UTC))
    ...
    def record_liquidation(self, event: PublicLiquidation) -> None:
        ...
        if event.timestamp > received_at + timedelta(seconds=5):
            raise ValueError("Future public liquidation timestamp")
        values = self._events.setdefault(event.symbol, deque())
        values.append(event)
        # An event timestamp cannot prove connection continuity. Only explicit
        # subscription/heartbeat observations advance the coverage endpoint.
    ...
    def _prune(self, now: datetime) -> None:
        ...
        for symbol, values in tuple(self._events.items()):
            retained = deque(event for event in values if event.timestamp > cutoff)
            if retained:

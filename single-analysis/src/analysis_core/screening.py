"""Signal-system screening: ten-candidate evidence pool, ten reviewed, one to three trend candidates selected."""
...
class Screening:
    ...
    async def select(self, cid, candidates):
        ...
        if len(pool) != 10:
            raise ValueError("筛选需要10个候选，当前不足；本轮分析失败")
        ...
        if len(bundles) != 10 or len({b.symbol for b in bundles}) != 10:
            raise ValueError("不足10份有效筛选证据；本轮分析失败，详见SCREENING_EVIDENCE")
        ...
        context = {
            ...
            "selection_minimum": 1,
            "selection_limit": 3,
            "ranking_objective": "十币中小长线方向置信度相对最高；低置信须如实披露",
        }
        schema = TrendScreeningResponse.model_json_schema()
        strict_schema(schema)
        document = await self.runner.request(
            analysis_id, context, schema, "screening_v3.md", event_prefix="SCREENING_MODEL"
        )
        ...
        if (
            ...
            or not 1 <= sum(item.get("status") == "SELECTED" for item in items) <= 3
        ):
        ...
        response = TrendScreeningResponse.model_validate(document)
        ...
        assessments = {a.symbol: a for a in response.assessments}
        return [
            dict(
                by_symbol[symbol].model_dump(mode="json"),
                selection_rank=assessments[symbol].selection_rank,
                screening_direction=assessments[symbol].direction,
                relative_confidence=assessments[symbol].value,
                ranking_rationale=assessments[symbol].ranking_rationale,
            )

...
class Executor:
    ...
    async def reconcile_entry(self, tid):
        ...
        # Hedge child fills are accounted even when very small; they have no TP/SL
        # until a later direction review. Do not invent protection prices for them.
        tp = sl = None
        if not d.get("hedge_child"):
            tp = tp_price(
                t["side"],
                entry,
                filled,
        ...
        active = [p for p in await self.exchange.positions(t["symbol"])
        ...
        self.store.update_trade(
            ...
            tp_price=str(tp) if tp is not None else None,
            sl_price=str(sl) if sl is not None else None,

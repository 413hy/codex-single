...
from longtime.vendor.models import CandidateRiskTag, Candle, ScannerConfig, Symbol
from longtime.vendor.public import (
    BybitInstrument,
    BybitOrderBook,
    BybitPublicClient,
    BybitTicker,
...
class CandidateFeatures(BaseModel):
    ...
    recent_range_expansion_ratio: float = Field(default=0, ge=0)
    latest_turnover_impulse_ratio: float = Field(default=0, ge=0)
...
class BybitUniverseScanner:
    ...
    async def scan(
        self, *, limit: int | None = None, exclude: set[str] | None = None
    ) -> ScanResult:
        ...
        eligible = [
            ...
            if symbol not in (exclude or set())
            and symbol in instrument_map
            and self._ticker_eligible(ticker)
        ]
        ...
        async def analyze(
            ...
            async with semaphore:
                ...
                try:
                    ...
                    return None
                try:
                    if not candles or datetime.now(UTC) - candles[-1].close_time > timedelta(
                        minutes=6
                    ):
                        raise ValueError("Stale scanner candles")
        ...
        ranked = _rank_candidates(
            unranked,
            len(unranked) if limit is None else limit,
            self._config.minimum_tradability_score,
        )
        return ScanResult(
    ...
    def _candidate(
        ...
        prior_range_median = statistics.median(ranges[-12:-3])
        recent_range_expansion = (
            statistics.fmean(ranges[-3:]) / prior_range_median if prior_range_median > 0 else 0.0
        )
        prior_turnover_median = statistics.median(
            float(candle.turnover) for candle in candles[-13:-1]
        )
        latest_turnover_impulse = (
            float(candles[-1].turnover) / prior_turnover_median
        ...
        features = CandidateFeatures(
            ...
            recent_range_expansion_ratio=max(0.0, recent_range_expansion),
            latest_turnover_impulse_ratio=max(0.0, latest_turnover_impulse),
        ...
        opportunity = (
            ...
            + abs(signed_return_30m) * 0.04
            + min(3.0, atr_activity) * 0.10
            + min(3.0, recent_range_expansion) * 0.10
            + min(3.0, latest_turnover_impulse) * 0.08
            + (1 - overlap) * 0.06
        ...
        reasons = (
            ...
            f"机会: 近30m变化 {signed_return_30m:.3f}%, 走势效率 {efficiency:.2f}",
            "转折活跃度: 近3柱振幅/前窗 "
            f"{recent_range_expansion:.2f}x, 最新成交额/中位 {latest_turnover_impulse:.2f}x",
...
def _missing_intervals(candles: tuple[Candle, ...]) -> int:
    ...
    for previous, current in itertools.pairwise(candles):
        ...
        if gap <= timedelta(0) or gap % expected != timedelta(0):
            raise ValueError("Invalid scanner candle interval")
        if gap > expected:
            missing += gap // expected - 1
    return missing
...
def _rank_candidates(
    ...
    eligible = [
        candidate for candidate in candidates if candidate.tradability_score >= minimum_tradability
    ]
    if not eligible or limit <= 0:
        return ()
    raw = [candidate.opportunity_raw for candidate in eligible]
    ...
    for candidate in eligible:
        ...
        risk_penalty = sum(
            {
                CandidateRiskTag.THIN_LIQUIDITY: 4.0,
                CandidateRiskTag.CHOPPY: 8.0,
                CandidateRiskTag.IMPACT_EXTENDED: 8.0,
                CandidateRiskTag.DATA_DEGRADED: 12.0,
            }[tag]
            for tag in candidate.risk_tags
        )
        # Movement finds opportunities; tradability and bounded risk penalties
        # keep unstable or difficult-to-execute symbols from dominating merely
        # because their latest candles were extreme.
        final = max(
            0.0,
            opportunity * 0.60 + candidate.tradability_score * 0.40 - risk_penalty,
        )

# auto-trader-longtime-02 — 独立交易执行系统
...
对冲系统：默认TP0.5U，对冲价格距离1.7U；条件限价反向开仓与顺向数量一致。部分成交保留TP；全部成交撤TP并等待公共信号。若顺向先止盈，撤余单并核对后平反向余仓。不把对冲转市价。有效重新判向先平逆向仓，再按当时价格设置TP与对冲。
...
systemctl status bybit-longtime-02.service

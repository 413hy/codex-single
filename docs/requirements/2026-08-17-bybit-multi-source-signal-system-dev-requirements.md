...
开发并部署一个 24/7 公开市场信号服务。每 30 分钟从 Bybit USDT 线性永续全量币池中筛选 5m 波动大、具备基本流动性且近端结构有机会的币，交给经三轮灰盒选出的 `gpt-5.6-terra / medium` 综合分析，再通过 Telegram 通知用户人工判断。

## 必须实现

- 动态全币池，不使用示例币硬白名单；
- 每轮 Top 5；每个成功定时周期恰好 2 个相对最优主信号，内部强度不决定通知名额；
- 上一轮主信号下一轮强制复核，退出/失效单独通知并停止实时监测；
- Bybit last 是参考价，mark 单列，Binance/OKX 只作可选公开旁证且不平均；
- 自建公开数据采集和证据构建，不直接运行或复制参考项目；
- 完成 1m/5m/15m/30m/1h/4h K 线、盘口快照、近期成交、OI、funding 和质量状态；
- 唯一方向、仅展示的大致止盈位、完成 K 线正式失效条件，以及形成中 15m/30m/1h 和下一根 15m 预测；大致止盈位不参与择优、阈值、freshness、生命周期或评测；
- 每轮一次批模型调用，严格 JSON Schema 和宿主二次校验；
- 轻量多指标实时阈值、迟滞、持续确认和每小时约 10 次软预算；阈值 crossing 只表示需要模型重新介入，不自动等于方向失效。同一分析/币种只接受首个 crossing，随即整套旧指标失效并持久化暂停，先通知，再重采并把触发原因交给模型；只有成功分析给出的新 analysis ID 阈值集合能覆盖并恢复监测，失败和重启都不得复活旧集合；模型与追踪复核结束后须再次深采两个主信号并按各指标最新基线激活阈值；

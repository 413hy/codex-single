...
日期：2026-08-18  
状态：用户已确认对话设计，等待书面规格复核  
范围：公开市场数据、Top 5 过滤、Codex 综合分析、Telegram 信号、实时阈值、紧急复核、结果审计  

## 1. 目标与边界

系统每个自然整点和半点执行一次固定流程：从 Bybit USDT 线性永续全市场中筛选 5 个适合山寨币超短线分析的币，采集完整公开市场数据，交给 `gpt-5.6-sol` 高推理强度综合分析，从当前 Top 5 中选出恰好 2 个最值得人工关注的主信号，通过 Telegram 通知用户。
...
flowchart TD
    ...
    MONITOR -->|阈值确认突破| FAST["立即 Telegram 阈值提醒\n说明触发原因和正在复核"]
    MONITOR -->|阈值确认突破| URGENT["紧急数据采集\n重新获取该币与市场背景"]
    URGENT --> CODEX_URGENT["Codex 独立紧急复核\n不等待定时任务"]
    TOOLS <--> CODEX_URGENT
    CODEX_URGENT --> TG_UPDATE["Telegram 正式复核结论"]
    CODEX_URGENT --> RULE_UPDATE["替换、保留或取消监测阈值"]
    RULE_UPDATE --> MONITOR
...
10. 成功周期原子保存证据、结论和阈值，并向 Telegram 发送恰好两条主信号及必要的追踪状态更新。
11. 实时监测器原子替换为当前两条主信号的有效阈值；掉出主信号集合的币停止实时监测。
...
宿主拒绝已穿越、距离过远、距离过近、语义重复、无法追溯或实现不支持的阈值。合理性使用当前 ATR、结构位置、spread、数据频率和市场噪声动态判断，不使用一个固定百分比覆盖所有山寨币。
...
- 使用迟滞、重新武装、语义族去重和有效期，防止边界抖动与旧阈值残留。
- 关键失效确认后不等待事件合并；普通同类事件默认合并 5 秒且不得超过 10 秒。
- 普通 Codex 唤醒保持约每小时 10 次软上限；关键失效和市场踩踏可以越过软上限。
...
5. 新结论原子替换、保留或取消旧阈值；失效/退出停止监测。

紧急 Codex 失败时保留原始阈值提醒并明确复核失败，不能表现为事件从未发生。

...
1. 当前生产 `monitoring.enabled=false`，保留半小时分析和 Telegram 主流程。
2. 完成代码、模型契约、Prompt/技能说明和测试后，先运行隔离影子环境。
3. 影子环境通过后重启生产，先验证一个完整固定周期。
4. 再开启实时监测，观察首个生产规则版本和后续事件。

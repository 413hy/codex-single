...
- 阈值数量按用途决定，通常每币 3–6 条、硬上限 8 条；关键事件立即提醒，普通事件短合并。
...
3. 配置当前安全底线、阈值硬上限、普通事件合并上限、工具轮数/调用数和新鲜度策略版本。
...
6. 保持 `gpt-5.6-sol`、`high`、ephemeral、read-only 和严格输出 Schema。
...
1. 校验当前观测、阈值、距离百分比/ATR、迟滞、确认周期、严重等级、有效期、实现支持和证据来源。
2. 拒绝已穿越、过远、过近、重复和无法追溯的阈值。
3. 每币允许多种语义阈值，去重后硬上限 8；不要求凑数量。
4. 新指令用最新实时值初始化基线；形成中 K 线只基线，完成柱才确认完成柱规则。
5. 新订阅后的第一根完成柱真实穿越必须触发；增加 ACU 端到端回归测试。
...
2. 指令切换、断线和重启后重新基线，取消旧币 pending 事件。
3. 关键失效立即进入事件队列；普通同族事件默认合并 5 秒且不超过 10 秒。
4. 普通唤醒保持约 10 次/小时软上限，关键失效和踩踏旁路；所有允许/拒绝原因入审计。
...
2. 紧急工作队列与定时工作队列分离；同币事件合并，不同关键币按受控并发处理。
3. 紧急采集重新获取该币和最新市场背景，调用程序 Codex 完整复核。
4. 正式结果补发维持、转弱、反向、失效或退出，并原子替换阈值。
5. 模型失败时发送复核失败状态，但保留原始事件提醒。

验收：定时模型运行期间触发关键事件，立即提醒仍在目标延迟内；紧急分析不等待定时锁；重复事件和重启不会重复投递。
